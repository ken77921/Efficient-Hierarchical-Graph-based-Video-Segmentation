...
Modifications Copyright (C) 2011, 2012
...
int main(int argc, char **argv) {
	...
	if (pDir != NULL) {
	 ...
	 bool x = true;
	 int frame_num = 0;
	 char video_path[1023];
	 sprintf(video_path,"%s/*.ppm",input_path);
	 const size_t cSize = strlen(video_path)+1;
	 wchar_t* file = new wchar_t[cSize];
	 mbstowcs (file, video_path, cSize);
	...
	for (int i = 0; i < frame_num; i++) {
		...
		for(int j=0;j<256;j++)
		{
			file_name[j]=(char)wctob(FindFileData.cFileName[j]);
			if(file_name[j]==0)
				break;
		}

		sprintf(filepath, "%s/%s", input_path, file_name);
		//sprintf(filepath, "%s/%05d.ppm", input_path, i + 1);
